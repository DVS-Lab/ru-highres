# OpenNeuro BOLD Voxel Resolution Benchmark

This analysis contains a reproducible Python workflow for characterizing the
spatial voxel resolution of raw BOLD fMRI NIfTI files in public OpenNeuro
datasets. It is a supporting analysis for the Rutgers high-resolution reward
fMRI manuscript, intended to contextualize reviewer concerns about whether the
study's acquisition should be considered high resolution.
...
- `results/dataset_resolution_single_band_summary.csv`
- `results/single_band_reference_rankings.csv`
...
## Acquisition-Type Stratification

The scanner also attempts to retrieve BIDS JSON sidecar metadata for each BOLD
object. It uses BIDS-style inheritance candidates from general to specific
sidecars, merges any JSON files found, and records the source sidecar keys in
the file-level audit table.

Acquisition type is classified conservatively:

- `multiband_or_sms`: explicit `MultibandAccelerationFactor` or
  `SliceAccelerationFactor` greater than 1, or sequence/protocol text indicating
  multiband/SMS.
- `likely_single_band_or_conventional_epi`: sidecar metadata was found and did
  not contain multiband/SMS evidence.
- `unknown`: sidecar metadata was missing or unavailable.

In-plane acceleration such as GRAPPA is recorded separately as
`inplane_acceleration_factor`; it is not treated as multiband acceleration. This
distinction is important for reviewer-response figures because multiband/SMS
datasets can occupy a different spatial/temporal tradeoff space than regular
single-band task fMRI.

`results/dataset_resolution_single_band_summary.csv` filters the primary
dataset-resolution rows to groups that are likely single-band/conventional EPI
and have no multiband/SMS or unknown files. `results/single_band_reference_rankings.csv`
compares the Rutgers high-resolution in-plane voxel size (1.75 mm) against that
filtered group. Smaller voxel sizes rank as finer resolution.
...
- QC flags: none
- Likely single-band/conventional pilot rows with sidecar metadata: 6
- Unknown acquisition-type pilot rows due to missing sidecars: 2
...
In the current pilot, the 1.75 mm Rutgers in-plane reference ranks first among
the six likely single-band/conventional dataset-resolution rows on both the LR
and AP axes. This pilot ranking is not a substitute for the full OpenNeuro scan,
but it verifies the ranking machinery and sidecar stratification.
